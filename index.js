...
class RocksLevel extends AbstractLevel {
  ...
  _openForReadOnly(options, callback) {
    const doOpen = () => {
      let columns;
      try {
        columns = binding.db_open_for_read_only(
          this[kContext],
          options,
          (err, columns) => {
            if (err) {
  ...
  openForReadOnly(options, callback) {
    ...
    // AbstractLevel의 상태 관리를 위해 _open으로 오버라이드
    const originalOpen = this._open;
    this._open = this._openForReadOnly.bind(this);

    this.open(options, (err) => {
      // 원래 _open 메서드로 복원
      this._open = originalOpen;
      callback?.(err);
    });
  }
